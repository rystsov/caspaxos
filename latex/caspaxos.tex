...
{\it How many crashes can we tolerate if the crashes occur while the reconfiguration is in progress?} The reconfiguration procedure is a sequence of atomic changes, at each step a system is in correct state either from perspective of previous or next configuration. So it can tolerate the number of failures the configuration allows. For example, during $A_1 \cdots A_{2F+1}$ to $A_1 \cdots A_{2F+2}$ reconfiguration the system can tolerate $F$ failures. On the $A_1 \cdots A_{2F+2}$ to $A_1 \cdots A_{2F+3}$ path the fault tolerance increases from $F$ to $F+1$.
...
The folowing behaviour helps CASPaxos achieve low latency:
\begin{itemize}
...
All storages were tested in the same environment. Gryadka, Etcd and MongoDB were using three nodes configuration deployed over wide area network in the Azure's\footnote{\href{https://azure.microsoft.com}{https://azure.microsoft.com}} datacenters in the "West US 2", "West Central US" and "Southeast Asia" regions running on DS4\_V2 machine with SSD drive.

Each node has a colocated client which in one thread in a loop was reading, incrementing and writing back a value. All clients used thier keys to avoid collisions. During the experiment latency (average duration of read-modify-write operation) was measured per each client (region).
...
As you can see the difference in the underlying consensus protocol yields significant difference in the performace of the system.

\subsection{Fault-tolerance}

EPaxos: single-master optimization can harm availability: if the master fails, the system cannot service requests until a new master is elected

EPaxos: With Multi-Paxos, or any variant that relies on a stable leader, a leader failure prevents the system from processing client requests until a new leader is elected. Although clients could direct their requests to another replica (after they time out), a replica will usually not try to become the new leader immediately

Analys predicted issue.

Experimental study\footnote{\href{https://github.com/rystsov/perseus}{https://github.com/rystsov/perseus}} reveals complete downtime of cluster in every Raft based system including Etcd, CockroachDB, Consul, RethinkDB; in Raft inspired\footnote{\href{https://www.mongodb.com/presentations/replication-election-and-consensus-algorithm-refinements-for-mongodb-3-2}{https://www.mongodb.com/presentations/replication-election-and-consensus-algorithm-refinements-for-mongodb-3-2}} MongoDB and in Paxos based Riak with consistent bucket.
