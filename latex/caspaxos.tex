...
\begin{longtable}{p{15em}|p{15em}} 
  ...
  If they all contain empty value then the proposer defines current state as $\emptyset$ otherwise it picks the value of the tuple with the highest ballot number as the current state.
  &
  If they all contain empty value then the proposer defines current state as $\emptyset$ otherwise it picks the value of the tuple with the highest ballot number as the current state.
  \\[6pt]
...
As you could noticed, the CASPaxos's state transition is almost identical to the Synod's initialization. 
...
\begin{enumerate}
  ...
  \item The garbage collection operation in background:
  \begin{enumerate}
...
Some of it is possible with Multi-Paxos or Raft and some isn't. The EPaxos paper emphasizes the following latency issue of leader-based consensus protocols:
