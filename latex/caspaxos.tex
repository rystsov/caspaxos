...
An experimental study\cite{raft} and the number of open source implementations\footnote{\href{https://raft.github.io/\#implementations}{https://raft.github.io/\#implementations}} indicate that Raft is understandable. However, its complexity is still comparable with Multi-Paxos: both systems\cite{chubby}\cite{raft} have several thousand of lines of code, both use concepts of leader election and leases, both are based on logs and require log compaction. CASPaxos is simpler: it doesn't have those pieces and our implementation\footnote{\href{https://github.com/gryadka/js}{https://github.com/gryadka/js}} is less than 500 lines of code.
...
The Raft paper\cite{raft} acknowledges that EPaxos\cite{epaxos} can achieve higher performance than Raft by using a leaderless approach and exploiting commutativity in state machine commands. A key-value storage with independent operations between keys looks like a good case for the EPaxos protocol. However, it adds significant complexity.
...
Gryadka\footnote{\href{https://github.com/gryadka/js}{https://github.com/gryadka/js}} is a prototype of a distributed key-value storage which uses that approach.
...
Log-based Paxos-like consensus protocols find wide adoption in the industry as a foundation of distributed databases. However many of them are key-value storages. CASPaxos is the consensus protocol which uses symmetric peer to peer approach to avoid the latency issues. The lightweight nature of CASPaxos allows to use it as a foundation of key-value storage with simple design and better resiliency guarantees.
...
{\bf Acknowledgements}. We thank Tobias Schottdorf and Greg Rogers for writing TLA+ specs for CASPaxos,Ezra Hoch for finding a flaw in the first version of the deletion process \ref{deletion} and Peter Bourgon for valuable comments and suggestions.
