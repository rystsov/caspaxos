...
TODO: But how many crashes can we tolerate if the crashes occur while the reconfiguration is in progress?
...
3 nodes deployment
each node has a colocated client
client in a loop reads, increments and writes back a value
each client uses thier key to avoid collision

EPaxos: when performing geo-replication, clients incur additional latency for communicating with a remote master

EPaxos: Multi-Paxos has high latency because the local replica must forward all commands to the stable leader

goal measure latency per region in a multi-region deployment
...
Bizur:  a single slow operation will increase the latency of all succeeding operations, until the slow operation is committed. For example, a network packet drop will affect multiple ongoing operations (instead of affecting just the operation within the dropped packet)

Analys predicted issue.

Experimental study\ref{\href{https://github.com/rystsov/perseus}{https://github.com/rystsov/perseus}} reveals complete downtime of cluster in every Raft based system including Etcd, CockroachDB, Consul, RethinkDB; in Raft inspired\ref{\href{https://www.mongodb.com/presentations/replication-election-and-consensus-algorithm-refinements-for-mongodb-3-2}{https://www.mongodb.com/presentations/replication-election-and-consensus-algorithm-refinements-for-mongodb-3-2}} MongoDB and in Paxos based Riak with consistent bucket.
