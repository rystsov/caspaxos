...
The same sequence executed in the reverse order shrinks cluster with even number of nodes.
...
Indeed without the mandatory re-scan, it's possible sequentially replace every acceptor with empty acceptor, lose all data and violate linearizability.
...
The update of the counters is necessary to avoid the lost update anomaly. It may happen when a concurrently updated value has lesser ballot number, and a reader prioritizes the tombstone over the new value.
